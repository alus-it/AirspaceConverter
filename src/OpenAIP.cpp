...
bool OpenAIP::ReadAirspaces(const std::string& fileName) {
	...
	if (!input.is_open() || input.bad()) {
		AirspaceConverter::LogError("Unable to open openAIP input file: " + fileName);
		return false;
	}
	AirspaceConverter::LogMessage("Reading openAIP file: " + fileName);
	ptree tree;
	read_xml(input, tree);
	input.close();
	ptree root;
...
bool OpenAIP::ReadWaypoints(const std::string& fileName) {
	...
	try {
		...
		double value = root.get_child("<xmlattr>").get<double>("DATAFORMAT");
		if (value != 1.1) {
			AirspaceConverter::LogError("DATAFORMAT attribute missing or not at the expected version 1.1", true);
			return false;
		}

		// Look for the 'root' of airports: WAYPOINTS tag
		ptree node = root.get_child_optional("WAYPOINTS");
		if (node != 0) wptFound=ParseAirports(node);

		// Look for the 'root' of navigation aids: NAVAIDS tag
		node = root.get_child_optional("NAVAIDS");
		if (node!=0) wptFound = wptFound || ParseNavAids(node);

		// Look for the 'root' of hot spots: HOTSPOTS tag
		node = root.get_child_optional("HOTSPOTS");
		if (node!=0) wptFound = wptFound || ParseHotSpots(node);
	} catch (...) {
		AirspaceConverter::LogError("Exception while parsing openAIP file: " + fileName, true);
		assert(false);
	}
	if(!wptFound) AirspaceConverter::LogMessage(".. Waypoints of any kind not found in this OpenAIP file: " + fileName, false);
	return wptFound;*/
	return false;
}
